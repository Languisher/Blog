...
$$
\forall (i, k) \in [\![1, M]\!] \times [\![1, K]\!], \quad C_{ik} = \sum_{j=1}^N A_{ij}B_{jk}
$$
我们可以看到有两种拆分方式：
（1）$M$ 和 $K$ 维度的拆分，对应拆分 $C_{i.}$ 和 $C_{.k}$ 的计算，每个 rank 负责一部分 $C_{ik}$。计算是相互独立的，除非要重建完整的矩阵，通信是没有必要的
（2）$N$ 维度的拆分，这意味着每个 rank 分别负责*所有*元素的一部分的求和计算。此时一定需要聚合通信，不然的话矩阵的计算结果就不正确。

第二种情况推导：假设设备为 $p$，分到一部分的 $J_{p} \subset \{1, \dots, N\}$
$$
C_{ik}^{(p)} = \sum_{j \in J_{p} }A_{ij }B_{jk},\quad C_{ik} = \sum_{p}C_{ik}^{(p)}
$$

...
参考文章：[ZeRO 和 FSDP：DP优化——将模型参数和中间状态分片到多卡](ZeRO%20和%20FSDP：DP优化——将模型参数和中间状态分片到多卡.md)
...
- 因此我们：**列切 $A$ 矩阵，行切 $B$ 矩阵**。推导请见 [三个矩阵乘法](../parallelism/矩阵乘法并行化.md#三个矩阵乘法)：$$A \to \begin{bmatrix}
A_{1}  & A_{2}
...
参考文章：[Pipeline Parallel：模型并行](Pipeline%20Parallel：模型并行.md)
## 并行度分配

...
 
我会跟踪以下指标：TTFT（首 token 时间，对用户感知的响应速度至关重要）、TPOT（每个输出 token 的时间，决定流式输出的平滑程度）、吞吐量（所有用户的 tokens/second）、GPU 利用率和显存利用率、有效吞吐（满足 SLO 的请求比例）、队列深度（用于检测系统是否过载），以及每百万 token 的成本。我会建立分位数（p50、p95、p99）监控面板，而不是只看平均值，因为真正影响用户体验的是尾延迟。此外，我还会分别跟踪 prefill 延迟和 decode 延迟，以便能够独立诊断各自的瓶颈。
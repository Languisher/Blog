...
传统 Prefill–Decode（PD）分离把 prompt 处理放在 P GPU，把逐 token 生成放在 D GPU，借此隔离 compute-bound prefill 对 memory-bound decode 的干扰。但典型协议只有 P→D 的 KV 传输：上一轮 response 是在 D 上生成的，KV 也留在 D；下一轮 prompt 若仍被送到 P，P 无法使用 D 上的状态，只能重新处理历史和新增文本，再把 KV 发回 D。多轮服务因此重复计算、重复传输，并可能把 P pool 或网络压垮。

论文区分 full prefill 与 append-prefill（AP）。前者没有可复用历史；后者只处理本轮新增的 `m` 个 token，并让它们关注已缓存的 `n` 个 token。当 `m` 远小于 `n` 时，AP 远轻于 full prefill。单 H100、Llama-3.1-8B 微基准中，一个 1,024-token full prefill 与 decode 共置使 batch-200 TPOT 慢约 48%，AP 只慢约 2%。因此 Turn 2+ 可以在持有 KV 的 D 上本地执行 AP，避免 P→D 回路。

但“所有 AP 都放 D”也不是无条件最优：它通常改善 Turn 2+ TTFT 和吞吐，却可能与 decode 争内存带宽、恶化 TPOT。PPD 用离线测量表比较两个极端——送 P 的 `x=0` 与留 D 的 `x=1`——再按 workload 网格和运营者给的 TTFT/TPOT 权重，对每个 Turn 2+ 请求做二元选择。
...
AMPD 不再规定多轮Agent每次新增prompt都必须去prefill GPU：它在tool result到达时比较prefill pool的TTFT slack、当前decoder的ITL slack、排队和KV传输成本，动态选择本地incremental prefill或远程prefill；同时用小窗口重排prefill队列，并离线联合规划P/D replicas与并行度。
...
2. **Online Adaptive Prefill Routing**
...
现有 PD disaggregation 把 prefill 和 decode 放到不同实例，但往往仍把所有 prefill 放进同一个队列和 batch。LAPS 进一步指出，prefill 自身也不是同一种工作：初始长 prompt 的 prefill 通常由大 GEMM 主导、偏 compute-bound；多轮会话每次只新增几十到几百个 token，却要读取很长历史 KV 的 re-prefill 更可能偏 memory-bound。两类请求混在一起，短请求会被长计算 convoy 阻塞，长请求也会受到短请求密集 KV 读写的干扰。

LAPS 在 prefill 层按一个运行时拟合的长度边界把请求分为 long-prefill 与 short-prefill 两类，维护两条队列，并保证一个执行时段/实例只执行一类。只有一个 prefill 实例时，两类 batch 在时间上互斥；有多个实例时，实例被划入 short/long pools，控制器依据队列、SLA 偏差和利用率一次迁移一个实例。这样做的目标不是简单地“短作业优先”，而是避免 compute-bound 和 memory-bound prefill 在同一个 batch/执行窗口中直接竞争。

短请求池还有第二层优化。LAPS 预先捕获 prompt-length × batch-size 的 CUDA Graph 网格，把请求按长度 bucket 化并 padding 到最近的 graph shape。Adaptive Wait-Depth scheduler 同时选择等待窗口 W 与目标 batch depth D：低负载时多等一点以凑出更大的 graph-aligned batch；临近 TTFT deadline 时立即发车；若没有合适 graph 则回退普通 prefill kernel。CUDA Graph 单独启用有时反而更慢，只有先做长短隔离、让短请求 shape 稳定且能聚批，graph launch 才更容易摊销。
...
Interactive LLM conversation平均可有22.4轮，下一轮需要所有历史KV；若每次都重算，论文workload中冗余计算最高占93.1%。本地低成本部署常用DRAM+SSD缓存history，但SSD慢且不同请求KV大小差异大。FCFS把SSD miss排在DRAM hit前时，GPU即使有可执行请求也会等待慢I/O；普通LRU又只看最近访问，而用户正在阅读长回答时conversation暂时不访问，语义上很可能回来却被当成冷数据。

Bidaw把DRAM-hit请求放ready queue，SSD-resident请求放preparing queue并先发I/O；ready按原arrival time保持公平，preparing以类似Highest Response Ratio Next的`等待时间/预计service`优先级兼顾小KV和aging，防止大KV饿死。Storage记录每user上一轮answer length；论文观察它与weighted reuse distance正相关，用分bucket统计与后台Belady-style ghost cache估计下一次访问的hit potential，替换DRAM中更不可能近期复用的数据。

进一步，MHA模型中某些中间tensor比KV更小但加载后可用部分计算恢复KV。

![](Attachments/Bidaw.png)

**Challenges and Insights**.
1. 真实多轮对话会产生大量长期存活、间歇访问的 KV，因此简单的 LRU/FIFO 难以有效管理 Host Memory。
	- Interactive LLM conversation平均可有 22.4 轮，下一轮需要所有历史 KV ；若每次都重算，论文workload中冗余计算最高占 93.1%
...
**System Design**.
1. **将请求分为 Ready Queue 和 Preparing Queue。** I/O-aware request scheduling：compute→看storage tier/size。
...
2. Bidaw 使用上一轮模型回答长度，预测下一次 KV 访问的 Weighted Reuse Distance 下界。
...
## KV$ Cache in the Wild (2506, ATC'25)


论文先用阿里云一周级 to-C 与 to-B production traces回答哪些 prefix KV 会在多久后被谁复用、要留多大 cache，再提出了一个基于 reuse-time exponential distribution, prefix spatial locality 的 workload-aware eviction 策略。


**Challenges and Insights**.
1. 对于 Production data 进行了分析，发现包括：
	- 真实生产 workload 中存在大量 KV Cache reuse，但复用率低于 synthetic workload 给出的结果。
	- KV Cache reuse 不仅来自多轮对话，单轮 API 请求同样是重要来源。
	- KV Cache reuse 主要发生在同一用户内部，跨用户复用非常少。
	- KV Cache reuse 高度偏斜，少数热点 block 和用户贡献了大部分收益。
	- 多轮请求的 turn 数和用户行为高度异构，但按 request category 分类后具有一定可预测性。
	- **KV Cache 具有很强但 workload-dependent 的 temporal locality**
	- 在给定 request category 和时间段后，KV reuse-time probability 可以用指数分布近似。
2. 对于基于 Production data 的 KV Cache Store 进行了分析，发现包括：
	- **KV Cache 的 spatial locality 集中在 prefix 的头部，而不是任意位置。**
	- 大多数 KV blocks 的有效 lifespan 很短，因此 LFU 容易受到历史频率污染。
	- 达到接近理想 hit rate 所需的 KV Cache 容量通常没有想象中大，但取决于 Attention 架构。

**System Design**.
1. Online Workload Profiler：
	- 按 request category 拟合 reuse-time distribution。
	- 根据未来 reuse probability，而不是单纯 recency 或 frequency，计算 block priority。
	- 使用 lifespan regulator，避免长尾概率模型长期保留已经死亡的 block。
	- 使用 per-workload priority queue
